...
{
	comp->west[W_TOP_LEFT] = (t_pixel){-(comp->letter_width + 1),(comp->letter_height / 2 + 1), comp->letter_color};
	comp->west[W_MB_LEFT] = (t_pixel){-(comp->letter_width / 2), -(comp->letter_height / 2 + 1), comp->letter_color};
	comp->west[W_TOP_MID]= (t_pixel){0, (comp->letter_height / 2 + 1), comp->letter_color};
	comp->west[W_MB_RIGHT] = (t_pixel){(comp->letter_width / 2), -(comp->letter_height / 2 + 1), comp->letter_color};
...
void	render_west_letter(t_win_glfw *win, t_compass *comp)
{
	...
	while (i < W_SIZE)
	{
		...
		backward_rotate_point(&west[i], comp->centre, -comp->sin_rad, -comp->cos_rad);
		i++;
	}
